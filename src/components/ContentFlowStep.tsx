...
function ContentFlowStep(props:propsSet) {
  ...
  return (
    <OuterComponent>
      <Typography sx={{ pb: 2 }} className="p">フェーズ: 0{props.phase + 1} | viewnumber: {props.viewnumber}</Typography>
      <Typography variant="h6" gutterBottom component="div" sx={{ borderBottom: '1px solid white' }}>{info.info}</Typography>
      <Typography variant="body1" gutterBottom component="div">{info.disc}</Typography>
    </OuterComponent>
  );
}
